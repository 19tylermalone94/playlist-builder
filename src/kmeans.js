...
export const useKmeansClusters = (userSelectedTracks) => {
  ...
  useEffect(() => {
    const fetchTracks = async () => {
      ...
      const data = csvParse(text, d => ({
        id: d.id,
        features: Object.fromEntries(Object.entries(d).filter(([key, _]) => key !== 'id').map(([key, value]) => [key, +value]))
      }));
      return data;
    };
    ...
    const runKmeans = async () => {
      ...
      const featureNames = Object.keys(userSelectedTracks[0].features);
      const centroids = normalize(userSelectedTracks, featureNames);
      const rawPopulation = normalize(data, featureNames);

      let clusters = Array(centroids.length).fill().map(() => []);
      for (let i = 0; i < 10; i++) {
        clusters = Array(centroids.length).fill().map(() => []);
        rawPopulation.forEach(track => {
          let closestCentroidIndex = 0;
          let minDistance = Infinity;
          centroids.forEach((centroid, index) => {
            const distance = euclideanDistance(track.features, centroid.features);
        ...
        centroids.forEach((centroid, index) => {
          if (clusters[index].length > 0) {
            centroid.features = averageFeatures(clusters[index], featureNames);
          }
        });
      }
